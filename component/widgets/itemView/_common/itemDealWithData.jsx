   ...
   if(data){
     if(!Array.isArray(data)){
       ...
       var k1 = data.id||'',
         ...
         if (data.url) k2 = <a href={v1} target="_blank">{k2}</a>
       ...
       function lazyimg(img, idf){
         if (img.indexOf('$$$')>-1){
           ...
           if (idf) return <li data-iid={idf} key={'img'+idf} data-imgsrc={img} className="himg-item lazyimg" title={k2}></li>
       ...
       if(k2&&k2!=''){
         if(React.isValidElement(k2)){
           headerDom = liDom
           ? <div className={"hheader"}>{k2}{liDom}</div>
           : <div className={"hheader"}>{k2}</div>
         }
         else{
           headerDom = liDom
           ? <div className={"hheader"}><a href={v1} target={'_blank'}>{k2}</a>{liDom}</div>
       ...
       if(bodys.length){
         if(data.img && k2.length) bodyDom = <div className={'hbody rebody'}>{bodys}</div>
         else{
