...
function mkmd(md_raw, templet, opts){
      ...
      if (!rev4.test(k)){
        if (accessVar.indexOf(k)>-1){
          if (rev3.test(v)){
            // 自定义css和js
            if (k==='css'||k==='js'){
              // if (v in CONFIG.mapper)
              if (k==='css' && _.has(CONFIG.mapper.pageCss, v)){
                cvariable[k] = '/css/'+CONFIG.mapper.pageCss[v]
              }
              if (k==='js' && _.has(CONFIG.mapper.pageJs, v)){
                cvariable[k] = '/js/'+CONFIG.mapper.pageJs[v]
              }
  ...
  return marked(md_raw, function (err, data) {
    return new Promise((res, rej) => {
      ...
      else mdcnt.mdcontent.desc = true
