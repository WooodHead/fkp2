...
// 生成js引用或者inner code
function createScript(doc, headElement, id, src){
...
function _inject() {
  ...
  if (!type || type==='css'){
    ...
    if(srcCode.indexOf('http')===0) {
      return createScript(doc, headElement, id, srcCode)
    } else if (srcCode.indexOf('/')===0) {
      if (srcCode.indexOf('/css/t/')>-1 || srcCode.indexOf('/js/t/')>-1) return createScript(doc, headElement, id, srcCode)
      var target = matchMapper(srcCode, 'js')
      if (target) return createScript(doc, headElement, id, srcCode)
    } else{
      srcCode = srcCode + "\n"; //增加末尾的换行符，方便在firebug下的查看。
      return createScript(doc, headElement, id, srcCode)
    }
